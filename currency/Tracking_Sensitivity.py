...
class TrackingSensitivity:
    def __init__(self, time_power_: dict):
        self.time_power = time_power_

    def power_at_current_time(self, time_sec_) -> int:
        minute = time_sec_ // 60
        if minute < 15:
            return 131
        elif minute < 21:
            return (minute - 15) * 3 + self.time_power["15"]
        elif minute < 43:
            return (minute - 21) + self.time_power["21"]
        else:
            return 171
    ...
    def get_all_cnr_from_gsv(self, _path_file_: str):
        all_gsv_cnr, epoch_valid_satellite_number = {}, {}

        time_sec, time_str = -1, ""
        begin = False
        for epoch_nmea in self.get_epoch_nmea(_path_file_):
            if epoch_nmea[0] is True:
                break
            epoch_gsv_info = {}
            for row in epoch_nmea[1]:
                if "GPGSV" in row:
                     epoch_gsv_info.update(self.parsing_gsv(row))
                elif "GGA" in row:
                    if begin is False:
                        if "E,1," not in row:
                            time_sec = -1
                            ...
                            begin = True
                    time_sec, time_str = self.get_time_from_gga(row)
                    if time_sec == -1:
                        break
                ...
                if sv_prn in all_gsv_cnr.keys():
                    if epoch_gsv_info[sv_prn][2] == -1:
                        if all_gsv_cnr[sv_prn][-1] != -1:
                            print("lose tracking time:", time_str, "single power ",
                                  self.power_at_current_time(time_sec),
    ...
    def tacking_analysis_base_gsv(self, _path_file_: str):
        all_cnr, valid_satellite_number = self.get_all_cnr_from_gsv(_path_file_)
        ...
        last_num = y[0]
        power__sv_num = {self.power_at_current_time(x[0]): last_num}
        for time_sec in x:
            current_sv_num = valid_satellite_number[time_sec]
            power_now = self.power_at_current_time(time_sec)
...
if __name__ == "__main__":
    ...
    path = "/home/ucchip/KWQ/gps_test/2022/0221/"
    ...
    test_run = TrackingSensitivity(time_power)
    for f in file_lst:
        print("\n\n", f)
        test_run.tacking_analysis_base_gsv(path + f)
